...
```{r}
breast_cancer_mod <- naiveBayes(diagnosis ~., data = breast_cancer_data) #using all the features from our dataset - put the . (period))
```
...
breast_cancer_data <- breast_cancer_data %>%
  mutate(predicted_diagnosis = predict(breast_cancer_mod, newdata = .)) ##mutate means adding a new column to the dataset
```
